...
import { Bot, Edit, Hash, Pencil, PencilIcon, PencilLine } from 'lucide-react'
    ...
    style={{ backgroundImage: `url(${bgImage})` }} >
     ...
     <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-[rgba(0,0,0,0.4)], bg-[hsla(210,100%,50%,0.15)] rounded-lg border border-gray-200'>
      <div className='flex items-center gap-3'>
        <Hash className='w-6 text-primary' />
        <h1 className='text-xl font-semibold text-white'>AI Blog Title Generator</h1>
      </div>
      <p className='mt-6 text-sm font-semibold text-gray-200'>Titles That Click, Every Time</p>

      <input onChange={(e)=> setInput(e.target.value)} value={input} type="text" className='w-full
       p-2 px-3 mt-2 outline-none text-sm rounded-md border bg-white border-gray-300'
      placeholder='Describe your blog vibe…' required />

      <p className='mt-4 text-sm font-medium text-gray-200'>Category</p>

      <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
      { blogCategories.map((item)=>(
        <span onClick={()=> setSelectedCategory(item)} 
        className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedCategory === item? 'bg-primary/50 text-white' 
     ...
     ${content ? "bg-[hsla(220,100%,20%,0.85)]" : "bg-[hsla(210,100%,50%,0.15)]" }
      ...
      <div className='flex items-center gap-3'>
         ...
         <h1 className='text-xl font-semibold text-white'>Your Head-Turning Titles Are Ready</h1>
      </div>

      {
        ...
        <div className='text-sm flex flex-col items-center gap-5 text-black'>
          ...
          <p className='text-primary font-semibold'>Catchy titles loading… hold onto your creativity</p>
        </div>
      </div>) 
